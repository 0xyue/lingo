...
export const getCourseProgress = cache(async () => {
    ...
    const unitsInActtiveCourse = await db.query.units.findMany({
    ...
    const firstUncompletedLesson = unitsInActtiveCourse
        .flatMap((unit) => unit.lessons)
        .find((lesson) => {
            return lesson.challenges.some((challenge) => {
                return !challenge.challengeProgress
                    || challenge.challengeProgress.length === 0
                    || challenge.challengeProgress.some((process) => process.completed === false);
            });
        });
